...
// // // // // // //     
    ...
    Cx_tot = 0.5*Cx_L+0.5*Cx_R + delta_Cx_lef*Sdlef + dXdQ*Q + 0*Cx_cross ;
    ...
    Cz_tot = 0.5*Cz_L+0.5*Cz_R + delta_Cz_lef*Sdlef + dZdQ*Q + 0*Cz_cross;
    ...
    Cm_tot = 0.5*Cm_L*eta_el_L +0.5*Cm_R*eta_el_R  + Cz_tot*(xcgr-xcg) + delta_Cm_lef*Sdlef + dMdQ*Q + 0.5*(delta_Cm_L+delta_Cm_R) + delta_Cm_ds + 0*Cm_cross;
    ...
    Cy_tot = 0.5*Cy_L+0.5*Cy_R + delta_Cy_lef*Sdlef + Cydail + delta_Cy_r30*drud + dYdR*R + dYdP*P + 0*Cy_cross;
    ...
    Cn_tot = 0.5*Cn_L+0.5*Cn_R + delta_Cn_lef*Sdlef - Cy_tot*(xcgr-xcg)*(cbar/B) + Cndail + delta_Cn_r30*drud + dNdR*R + dNdP*P + 0.5*(delta_Cnbeta_L+delta_Cnbeta_R)*beta + 0*Cn_cross;
    ...
    Cl_tot = 0.5*Cl_L+0.5*Cl_R + delta_Cl_lef*Sdlef + Cldail + delta_Cl_r30*drud + dLdR*R + dLdP*P + 0.5*(delta_Clbeta_L+delta_Clbeta_R)*beta + 0*Cl_cross;
...
// // // // // // // // //         /* HIFI Table */
        ...
        hifi_other_coeffs(alpha,Sel,temp);
            ...
            /*  Coeff debuging*/
              printf(" Cm*eta_el %f  Vs 0.5*Cm_L*eta_el_L %f Vs 0.5*Cm_R*eta_el_R %f Vs  0.5*(Cm_L*eta_el_L + Cm_R*eta_el_R) %f\n", Cm*eta_el ,0.5*Cm_L*eta_el_L, 0.5*Cm_R*eta_el_R, 0.5*(Cm_L*eta_el_L + Cm_R*eta_el_R) );
            /* end debuging*/
    /* XXXXXXXX Cx_tot XXXXXXXX */

    dXdQ = (cbar/(2*vt))*(Cxq + delta_Cxq_lef*Sdlef);

    Cx_tot = 0.5*Cx_L+0.5*Cx_R + delta_Cx_lef*Sdlef + dXdQ*Q;

       /* ZZZZZZZZ Cz_tot ZZZZZZZZ */ 

    dZdQ = (cbar/(2*vt))*(Czq + delta_Cz_lef*Sdlef);

    Cz_tot = 0.5*Cz_L+0.5*Cz_R + delta_Cz_lef*Sdlef + dZdQ*Q;

       /* MMMMMMMM Cm_tot MMMMMMMM */ 

    dMdQ = (cbar/(2*vt))*(Cmq + delta_Cmq_lef*Sdlef);

    Cm_tot = 0.5*(Cm_L*eta_el_L + Cm_R*eta_el_R) + Cz_tot*(xcgr-xcg) + delta_Cm_lef*Sdlef + dMdQ*Q + delta_Cm + delta_Cm_ds;
    ...
    Cy_tot = 0.5*Cy_L+0.5*Cy_R + delta_Cy_lef*Sdlef + dYdail*Adail + delta_Cy_r30*drud + dYdR*R + dYdP*P;
    ...
    Cn_tot = 0.5*Cn_L+0.5*Cn_R + delta_Cn_lef*Sdlef - Cy_tot*(xcgr-xcg)*(cbar/B) + dNdail*Adail + delta_Cn_r30*drud + dNdR*R + dNdP*P + delta_Cnbeta*beta;
    ...
    Cl_tot = 0.5*Cl_L+0.5*Cl_R + delta_Cl_lef*Sdlef + dLdail*Adail + delta_Cl_r30*drud + dLdR*R + dLdP*P + delta_Clbeta*beta;
