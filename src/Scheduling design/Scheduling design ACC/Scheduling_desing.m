...
Scheduling_map_ACC.tau_ACC = NaN*ones(size(M_sq_sche,2),size(H_sq_sche,2));
Scheduling_map_ACC.tau_q   = NaN*ones(size(M_sq_sche,2),size(H_sq_sche,2));
Scheduling_map_ACC.K_b_ACC = NaN*ones(size(M_sq_sche,2),size(H_sq_sche,2));
Scheduling_map_ACC.K_b_q   = NaN*ones(size(M_sq_sche,2),size(H_sq_sche,2));
%% Go interatively
for I=1:size(M_sq_sche,2)
    for J=1:size(H_sq_sche,2)
        if isnan(M_eval_sche(I,J))==0 && isnan( Scheduling_map_ACC.tau_ACC(I,J)  ) && M_eval_sche(I,J) >= 0.4*min( M_eval_sche(:,J) )
%             return
            ...
            Scheduling_map_ACC.tau_ACC(I,J) = params_opt(1);
            Scheduling_map_ACC.tau_q(I,J) = params_opt(2);
            Scheduling_map_ACC.K_b_ACC(I,J) = params_opt(3);
            Scheduling_map_ACC.K_b_q(I,J) = params_opt(4);
            close all
        end
    end
end
...
surf(M_eval_sche,H_eval_sche,Scheduling_map_ACC.K_b_q)
% return
% tau_ACC
[fitresult, gof] = createFit1(M_eval_sche, H_eval_sche, Scheduling_map_ACC.tau_ACC);
tau_ACC_extrap = NaN*ones(size(M_sq_sche,2),size(H_sq_sche,2));
for I=1:size(M_sq_sche,2)
    for J=1:size(H_sq_sche,2)
        if isnan(M_eval_sche(I,J))==0 %&& M_eval_sche(I,J) <= 2*min( M_eval_sche(:,J) )
%             if M_eval_sche(I,J)<=1 
                tau_ACC_extrap(I,J) = fitresult( M_eval_sche(I,J) , H_eval_sche(I,J));
%             elseif M_eval_sche(I,J)>1 && J<=9
%                 tau_ACC_extrap(I,J) = fitresult( M_eval_sche(11,J) , H_eval_sche(11,J));
%             elseif J>9
%                 tau_ACC_extrap(I,J) = tau_ACC_extrap(I,9);
%             end
        end
    end
end
...
close all
% tau_q
[fitresult, gof] = createFit1(M_eval_sche, H_eval_sche, Scheduling_map_ACC.tau_q);
...
% K_b_ACC
[fitresult, gof] = createFit1(M_eval_sche, H_eval_sche, Scheduling_map_ACC.K_b_ACC);
K_b_ACC_extrap = NaN*ones(size(M_sq_sche,2),size(H_sq_sche,2));
for I=1:size(M_sq_sche,2)
    for J=1:size(H_sq_sche,2)
        min( M_eval_sche(I,:) )
        if isnan(M_eval_sche(I,J))==0 %&& M_eval_sche(I,J) <= 2*min( M_eval_sche(:,J) )
%             if M_eval_sche(I,J)<=1 
                K_b_ACC_extrap(I,J) = fitresult( M_eval_sche(I,J) , H_eval_sche(I,J));
%             elseif M_eval_sche(I,J)>1 && J<=9
%                 K_b_ACC_extrap(I,J) = fitresult( M_eval_sche(11,J) , H_eval_sche(11,J));
%             elseif J>9
%                 K_b_ACC_extrap(I,J) = K_b_ACC_extrap(I,9);
%             end
        end
    end
end
...
close all
% K_b_q
[fitresult, gof] = createFit1(M_eval_sche, H_eval_sche, Scheduling_map_ACC.K_b_q);
...
% tau_ACC_extrap_filter = imfilter(tau_ACC_extrap,h,'replicate');
% surf(M_eval_sche,H_eval_sche,tau_ACC_extrap);
% hold on
% plot(F_envelope.M_1g,F_envelope.H_1g)
% figure
% surf(M_eval_sche,H_eval_sche,tau_ACC_extrap_filter);
% view(0,90)
% hold on
% plot(F_envelope.M_1g,F_envelope.H_1g)
%%
...
% aproaching stall
nanmean( nanmean(Scheduling_map_ACC.K_b_ACC) )
nanmean( nanmean(Scheduling_map_ACC.K_b_q) )
...
close all
    % ACC
    [fitresult, gof] = Poly_fit(M_eval_sche, H_eval_sche, tau_ACC_extrap);
    tau_ACC_extrap_smooth = NaN*ones(size(M_sq_sche,2),size(H_sq_sche,2));
    for I=1:size(M_sq_sche,2)
        for J=1:size(H_sq_sche,2)
            tau_ACC_extrap_smooth(I,J)= fitresult( M_sq_sche(I) , H_sq_sche(J));
            M_eval_sche_unc(I,J) = M_sq_sche(I);
            H_eval_sche_unc(I,J) = H_sq_sche(J);
        end
    end
    surf(M_eval_sche_unc,H_eval_sche_unc,tau_ACC_extrap_smooth);
    pause
    close all
    % q
