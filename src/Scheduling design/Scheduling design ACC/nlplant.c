...
#define XU prhs[0]
#define XDOTY plhs[0]
#define INPUTSIZE 19 // Total size of all inputs to the function
...
if (mxGetM(XU)==INPUTSIZE && mxGetN(XU)==1){ 
      ...
      /* Calling Program */
      xup = mxGetPr(XU);
      XDOTY = mxCreateDoubleMatrix(INPUTSIZE, 1, mxREAL);
      xdotp = mxGetPr(XDOTY);
...
/*NasaData        %translated via eq. 2.4-6 on pg 80 of Stevens and Lewis*/
  ...
  double Throttle, el, ail, rud, dail, drud, lef, dlef;
  double qbar, mach, ps;
  ...
  el    = xu[14];   /* Elevator setting in degrees. */
  ail   = xu[15];   /* Ailerons mex setting in degrees. */
  rud   = xu[16];   /* Rudder setting in degrees. */
  lef   = xu[17];   /* Leading edge flap setting in degrees */
  
  fi_flag = xu[18]/1;       /* fi_flag */
  ...
  /* %%%%%%%%%%%%%%%%%%
     Atmospheric effects
     sets dynamic pressure, mach number and gravity correction
     %%%%%%%%%%%%%%%%%% */
