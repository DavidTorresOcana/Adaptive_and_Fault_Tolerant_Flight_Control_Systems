...
void nlplant(double *xu, double *xdot){
  ...
  * Deviations
  ...
  double m    = xu[23]; //9295.44; /*TBC*/        /* mass, kg */
  double S_current_L  = xu[24];// 27.87/2; /*TBC*/           /* Left planform area, m^2 */
  double S_current_R    = xu[25]; //27.87/2; /*TBC*/           /* Right planform area, m^2 */
  double S_current = S_current_L + S_current_R;
  double S_fin_current    = xu[26]; //6.578; /*TBC*/           /* Fin area, m^2 */
  double xcg  =  xu[27]; //0.30; /*TBC*/     /* center of gravity as a fraction of cbar. */
  double Jy  =  xu[28]; //75673.6; /*TBC*/          /* kg-m^2 */ 
  double Jxz =  xu[29]; //1331.4;  /*TBC*/           /* kg-m^2 */   
  double Jz  = xu[30]; //85552.1; /*TBC*/          /* kg-m^2 */
  double Jx  = xu[31]; //12874.8; /*TBC*/           /* kg-m^2 */
  double EFF_el_L = xu[32] , EFF_el_R = xu[33], EFF_ail_L = xu[34] , EFF_ail_R = xu[35], EFF_rud = xu[36], EFF_lef_L = xu[37], EFF_lef_R= xu[38]; /* [0-1] Dealing with surface Efficientcy: EFF_ represent a effectiveness or remaining surface compared with the normal surface*/
  double Delta_C_Lift_X = xu[39], Delta_C_Drag_X = xu[40], Delta_C_m_X = xu[41]; /* Dealing with Change in draging properties: Treat as increments*/
  /*########################################*/
  /*########################################*/
  ...
  double y_AC = y_CMA_R - y_CMA_L; /* Position of the AC*/
...
// //     Cydail = (delta_Cy_a20 + delta_Cy_a20_lef*Sdlef)*Adail; 
    ...
    Cx_tot = Cx_X+Cx_D + delta_Cx_lef*Sdlef + dXdQ*Q;

       /* ZZZZZZZZ Cz_tot ZZZZZZZZ */ 

    dZdQ = (cbar/(2*vt))*(Czq + delta_Cz_lef*Sdlef);

    Cz_tot = Cz_X+Cz_D + delta_Cz_lef*Sdlef + dZdQ*Q;

       /* MMMMMMMM Cm_tot MMMMMMMM */ 

    dMdQ = (cbar/(2*vt))*(Cmq + delta_Cmq_lef*Sdlef);

    Cm_tot = Cm_X*eta_el_X + ( Cm*eta_el - Cm_X*eta_el_X)+  + Cz_tot*(xcgr-xcg) + delta_Cm_lef*Sdlef + dMdQ*Q + delta_Cm + delta_Cm_ds;
    ...
    Cy_tot = Cy_X+Cy_D + delta_Cy_lef*Sdlef + dYdail*Adail + delta_Cy_r30*drud + dYdR*R + dYdP*P;
    ...
    Cn_tot = Cn_X+Cn_D + delta_Cn_lef*Sdlef - Cy_tot*(xcgr-xcg)*(cbar/B) + dNdail*Adail + delta_Cn_r30*drud + dNdR*R + dNdP*P + delta_Cnbeta*beta;
    ...
    Cl_tot = Cl_X+Cl_D + delta_Cl_lef*Sdlef + dLdail*Adail + delta_Cl_r30*drud + dLdR*R + dLdP*P + delta_Clbeta*beta;
